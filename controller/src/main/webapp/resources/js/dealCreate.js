...
function onResetAllClick() {
    companyNewChange(true);
    contactNewChange(true);
}

function contactNewChange(value) {
    var groups = document.getElementsByClassName("form-group contactNew");
    for (var i = 0; i < groups.length; i++) {
        groups[i].hidden = !value;
    }
    groups = document.getElementsByClassName("form-group contactExist");
    for (i = 0; i < groups.length; i++) {
        groups[i].hidden = value;
    }
}

function companyNewChange(value) {
    var groups = document.getElementsByClassName("form-group companyNew");
    for (var i = 0; i < groups.length; i++) {
        groups[i].hidden = !value;
    }
    groups = document.getElementsByClassName("form-group companyExist");
    for (i = 0; i < groups.length; i++) {
        groups[i].hidden = value;
    }
}
